...
const EnclosureDetailModal = ({ enclosure, authToken, onClose, onChanged }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50">
            <div
                className="bg-white w-full sm:max-w-sm rounded-t-2xl sm:rounded-2xl p-4 max-h-[85vh] flex flex-col"
                style={{ paddingBottom: 'calc(1rem + env(safe-area-inset-bottom))' }}
            >
                <div className="flex items-center justify-between mb-2">
                    <h2 className="text-lg font-bold text-gray-800 truncate">{enclosure.name}</h2>
                ...
                {loading ? (
                    ...
                    <>
                        ...
                        <div className="flex-1 overflow-y-auto space-y-1.5">
                            {unassigned.length === 0 && <p className="text-xs text-gray-400 text-center py-6">No matching animals.</p>}
                            {unassigned.map((a) => (
                                <button
                                    key={a.id_public}
                                    onClick={() => assign(a)}
                                    disabled={busyId === a.id_public}
                                    className="w-full flex items-center gap-2 p-2 rounded-lg bg-gray-50 text-left disabled:opacity-50"
                                >
                                    <div className="w-8 h-8 rounded-full overflow-hidden bg-gray-100 flex-shrink-0">
                                        <AnimalImage src={a.imageUrl || a.photoUrl} alt={a.name} iconSize={12} />
                                    </div>
                                    <span className="text-sm flex-1 truncate">{[a.prefix, a.name, a.suffix].filter(Boolean).join(' ')}</span>
                                    {busyId === a.id_public ? <Loader2 size={14} className="animate-spin" /> : <Plus size={14} className="text-accent" />}
                                </button>
                            ))}
                        </div>
                        <button onClick={() => setShowPicker(false)} className="mt-2 text-xs text-gray-500 font-semibold">Back to assigned list</button>
                    </>
                ) : (
                    <>
                        <div className="flex-1 overflow-y-auto space-y-1.5">
                            {assigned.length === 0 && (
                                <div className="text-center py-8 text-gray-400 text-sm flex flex-col items-center gap-1.5">
                                    <Home size={22} />
                                    No animals assigned yet.
                                </div>
                            )}
                            {assigned.map((a) => (
                                <div key={a.id_public} className="flex items-center gap-2 p-2 rounded-lg bg-gray-50">
                                    <div className="w-8 h-8 rounded-full overflow-hidden bg-gray-100 flex-shrink-0">
                                        <AnimalImage src={a.imageUrl || a.photoUrl} alt={a.name} iconSize={12} />
                                    </div>
                                    <span className="text-sm flex-1 truncate">{[a.prefix, a.name, a.suffix].filter(Boolean).join(' ')}</span>
                                    <button onClick={() => remove(a)} disabled={busyId === a.id_public} className="p-1 text-red-400 disabled:opacity-50">
                        ...
                        <button
                            ...
                            className="mt-2 flex items-center justify-center gap-1.5 bg-accent text-white font-semibold py-2 rounded-lg text-sm"
                        >
