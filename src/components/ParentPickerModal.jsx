            ...
            <div className="bg-white dark:bg-dark-card-bg w-full max-w-sm rounded-2xl flex flex-col max-h-[85vh] overflow-hidden">
                ...
                <div className="overflow-y-auto px-4 py-3 space-y-1" style={{ paddingBottom: 'calc(0.75rem + env(safe-area-inset-bottom))' }}>
                    {loading ? (
                        ...
                        <p className="text-center text-xs text-gray-400 dark:text-dark-text-muted py-8">Search your animals or the public directory.</p>
                    ) : results.length === 0 ? (
                        <p className="text-center text-xs text-gray-400 dark:text-dark-text-muted py-8">No matching animals found.</p>
                    ) : results.map((a) => (
                        <button
                            key={a.id_public}
                            type="button"
                        ...
                        >
                            <div className="w-11 h-11 rounded-lg overflow-hidden bg-gray-100 dark:bg-dark-surface flex-shrink-0">
                            ...
                            <div className="min-w-0 flex-1">
                                ...
                                <p className="text-xs text-gray-400 dark:text-dark-text-muted">{a.species} · {a.gender} · {a.id_public}</p>
                            </div>
