            ...
            <div className="bg-white w-full max-w-sm rounded-2xl flex flex-col max-h-[85vh] overflow-hidden">
                ...
                <div className="overflow-y-auto px-4 py-3 space-y-1" style={{ paddingBottom: 'calc(0.75rem + env(safe-area-inset-bottom))' }}>
                    {loading ? (
                        ...
                        <p className="text-center text-xs text-gray-400 py-8">Search by breeder/owner name or CTUID.</p>
                    ) : results.length === 0 ? (
                        <p className="text-center text-xs text-gray-400 py-8">No matching profiles found.</p>
                    ) : results.map((p) => (
                        <button
                            key={p.id_public}
                            type="button"
                        ...
                        >
                            <div className="min-w-0 flex-1">
                                <p className="text-sm font-semibold text-gray-800 truncate">{p.breederName || p.personalName || 'Unnamed'}</p>
                                <p className="text-xs text-gray-400">{p.id_public}</p>
                            </div>
                        </button>
                    ))}
                </div>
